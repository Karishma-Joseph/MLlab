...
def decision_tree(data, attributes):
    ...
    labels = set(classList)
    for x in labels:
        if classList.count(x) == len(classList):
            return x

    maxGainNode = attribute_selection(data)
    root = attributes[maxGainNode]

    theTree = {root: {}}
    del(attributes[maxGainNode])
    ...
    for value in uniqueVals:
        ...
        theTree[root][value] = decision_tree(dataset_split(data, maxGainNode, value), subAttributes)
    return theTree
